...
class VerticalGridRenderer extends _rendering_1.BaseRenderer {
    ...
    update(context, container) {
        ...
        while (idx++ < _config_1.default.grid.time.max * 2) {
            ...
            const [line, lineState] = this.get('x_gridline' + idx, () => _rendering_1.GraphicUtils.createLine([0, 0], [0, height], this.lineStyle));
            if (lineState.new)
                container.addChild(line);
            line.position.set(x, 0);
            line.height = height;
            const [text, textState] = this.get('x_gridtext' + idx, () => _rendering_1.GraphicUtils.createText(_rendering_2.DateUtils.formatUnixTSToHHmm(time), [x, height], this.textStyle, 1.1));
            if (textState.new)
                container.addChild(text);
            text.position.set(x, height);
            text.text = _rendering_2.DateUtils.formatUnixTSToHHmm(time);
        }
        return container;
    }
}
